...
def main():
    ...
    for i in range(len(dm)):
        ...
        if len(dm) % split == 0:
            break
    # Creates the empty encoding matrix to be filled in with the correct values
    # and the matrix to be solved by RREF_Calculator
    encodingMatrix = np.empty([split, split], dtype='float')
    firstColtoRREF = np.array(
        [[dm[0], dm[1], dm[2], em[0]], [dm[3], dm[4], dm[5], em[split]], [dm[6], dm[7], dm[8], em[split * 2]]],
        dtype='float')
    secondColtoRREF = np.array(
        [[dm[0], dm[1], dm[2], em[1]], [dm[3], dm[4], dm[5], em[4]], [dm[6], dm[7], dm[8], em[7]]], dtype='float')
    thirdColtoRREF = np.array(
        [[dm[0], dm[1], dm[2], em[2]], [dm[3], dm[4], dm[5], em[5]], [dm[6], dm[7], dm[8], em[8]]], dtype='float')
