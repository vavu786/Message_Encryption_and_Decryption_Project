...
if conditionToEncode.lower() == "e":
    ...
    while messageSize % messageSplit != 0:
        condOne = input("The encoding process cannot occur since the size of the encoding matrix was not a factor of the message size. Please either change the encoding matrix dimension(enter an E) or change the message matrix dimension(enter an M): ")
        while condOne.lower() != "e" and condOne.lower() != "m":
            cond1 = input("Please enter either an E or an M: ")
        if condOne.lower() == "e":
            messageSplit = int(input("How many rows should the square encoding matrix have?(it should be a factor of the size of the message): "))
        if condOne.lower() == "m":
